...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-sky-100 bg-white/90 px-4 py-5 shadow-[18px_0_50px_rgba(14,165,233,0.08)] backdrop-blur-2xl lg:block">
      <div className="absolute inset-y-0 right-0 w-px bg-gradient-to-b from-transparent via-secondary/40 to-transparent" />
      <Logo compact />

      <div className="mt-6 rounded-2xl border border-sky-100 bg-gradient-to-br from-cyan-soft via-white to-white p-4 shadow-subtle">
      ...
      <nav className="mt-8 space-y-7">
        {sections.map((section) => (
          <div key={section.title}>
            <p className="mb-2 px-3 text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">{section.title}</p>
            <div className="space-y-1.5">
              {section.items.map(({ label, to, icon: Icon, end }) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    cn(
                      'group relative flex h-12 items-center gap-3 overflow-hidden rounded-xl px-3 text-sm font-bold transition-all duration-200',
                      isActive
